...
getAtom(nextType,paramList);
//alert(JSON.stringify(nextType.substring(0,1)));

// now i need to find the vertices for each atom.
  } // end atomList loop  
...
function getAtom(thisType,thisParamList) {
  ...
  if (thisType.substring(0,1) === "S") { 
    thisShape = getSheet(thisParamList[0],thisParamList[1],thisParamList[2]);
alert(thisShape);
  } else if (thisType.substring(0,1) === "H") {
...
//  alert(JSON.stringify([handle, crosscap, cone, kali]));
  ...
  //alert(JSON.stringify([6,newCone,newKali,infArray,atomList]));
    ...
    if ((newCone.length === 1) && (newKali.length === 1)) { // just a hat
      hatName = findHat(newCone[0],newKali[0]);
      atomList.push([hatName,newCone[0],newKali[0]]);
    } else { // plural hats
      for (let i=0; i<newCone.length; i++) { // step 7. cone & kali (hat -> sheet)
        if ((i === newCone.length-1) && (newKali.length === 1)) { // last hat
          hatName = findHat(newCone[i],newKali[0]);
          atomList.push([hatName,newCone[i],newKali[0]]);
        } else {
          infCount--;
          hatName = findHat(newCone[i],infCount);
          atomList.push([hatName,newCone[i],infCount]);
          newKali.push(infCount);
...
// begin with ultra parallels in list. help.
function getSheet(input1,input2,input3) {
  let myShape = [];
  if (input3<0) { // all ultra parallel
alert(0);
    myShape = solve0Ang(input1,input2,input3);
  } else if (input2<0) { // two ultra parallel
alert(1);
    myShape = solve1Ang(input1,input2,Math.PI/input3);
  } else if (input1<0) { // one ultra parallel
alert(2);
    myShape = solve2Ang(input1,Math.PI/input2,Math.PI/input3);
  } else { // none ultra parallel
alert(3);
    myShape = solve3Ang(Math.PI/input1,Math.PI/input2,Math.PI/input3);
  }
  return(myShape);
} // end getSheet()
...
function findHat(A,B) {
  let negA = (A<0);
  let negB = (B<0);
  return("H"+(3+1*negA+2*negB));
}
...
//alert(JSON.stringify(reflMat2));
let result = multMatMat(reflMat2,reflMat1);
