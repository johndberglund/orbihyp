...
var atomType = 1; // 0=sheet, 1=hat, 2=pillow
...
// why am I drawing here? do it in draw()...?
  ...
  document.getElementById("orbifold").value="wow";
...
alert(numSides);
  switch(numSides) {
    ...
    case 4: // side1 * angle2.
      side1 = input1;
      angle2 = Math.PI/input2;
      funDom = solve2Ang(side1,angle2/2,angle2/2);
      genMats = [];
    ...
    case 5: // angle1 * side2 (here side1 is opposite angle1)
      angle1 = 2*Math.PI/input1;
      side1 = input2;
      side2 = Math.asinh(Math.sqrt((1+Math.cos(angle1))/(Math.cosh(side1)-1)));
      funDom = solve1Ang(side2,side2,angle1);
      genMats = [];
    ...
    case 6: // side1 * side2 (here side3 is opposite side1)
      side1 = input1;
      let side3 = input2;
      side2 = Math.asinh(Math.sqrt((Math.cosh(side1)+1)/(Math.cosh(side3)-1)));
      funDom = solve0Ang(side2,side2,side1);
      genMats = [];
      myLine = points2Line(funDom[0],funDom[1]);
      myMat = hReflMat(myLine);
      genMats.push(myMat);
...
function alertLen(polygon) {
  let lenList = [hDist(polygon[0],polygon[polygon.length-1])];

  for (i=0;i<polygon.length-1;i++) {
    lenList.push(hDist(polygon[i],polygon[i+1]));
  }
