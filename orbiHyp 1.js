...
//let blah = [Math.sqrt(2),1,0];
//let blah = cross([3,2,2],[Math.sqrt(2),1,0]);
//alert(blah);
...
function regPolyVertices(P,Q) {
  ...
  for (let i = 0; i<P; i++) {
    ...
    let x = Math.sinh(radius)*Math.cos(i*2*Math.PI/P);
    let y = Math.sinh(radius)*Math.sin(i*2*Math.PI/P);
    vertices.push([t,x,y]);
  }
...
// multiply a matrix times a vector to get a vector.
...
/*
// given two hyperboloid points, draw segment on screen.
function drawSeg(P,Q,context) {
  let scrP = pt2Screen(P,hZoom);
  let scrQ = pt2Screen(Q,hZoom);
//short version
//  context.moveTo(scrP[0],scrP[1]);
//  context.lineTo(scrQ[0],scrQ[1]);
//  context.stroke();
// To parameterize. U and V are a basis for the plane through P,Q & origin.
  let R = normalize(cross(P,Q));
  let U = cross(R,[1,0,0]);
  let V = cross(R,U);
//  alert(JSON.stringify([P,Q,R,U,V]));
 // let pParam = Math.log(P[0]*V[0]+Math.sqrt(P[0]*P[0]*V[0]*V[0]-1));
//  let qParam = Math.log(Q[0]*V[0]+Math.sqrt(Q[0]*Q[0]*V[0]*V[0]-1));
// these above are negative - cant take log
...
let vertices = regPolyVertices(8,3);
let polyLeng = vertices.length;
let scrVertices = ptList2Screen(vertices,hZoom);

//drawSeg(vertices[0],vertices[1],context);
